...
function parseColor(str: string) {
  ...
  if (str.startsWith("rgb")) {
    ...
    if (values && values.length >= 3)
      return {
        ...rgbToHsv(values[0], values[1], values[2]),
        a: values[3] ?? 1,
      };
  }
...
function hsvToOklchString(h: number, s: number, v: number, a: number) {
  ...
  if (allowOpacity && a < 1)
    return `oklch(${L} ${C} ${H} / ${parseFloat(a.toFixed(2))})`;
  return `oklch(${L} ${C} ${H})`;
}
...
function hsvToHslString(h: number, s: number, v: number, a: number) {
  ...
  let sHsl =
    l && l < 1 ? (sNorm * vNorm) / (l < 0.5 ? l * 2 : 2 - l * 2) : sNorm;
  if (allowOpacity && a < 1)
...
>
  ...
  >
    <div
      class="absolute inset-0 bg-gradient-to-r from-white to-transparent pointer-events-none"
    ></div>
    <div
  ...
  <div class="flex gap-3 items-center">
    ...
    >
      <div
        ...
        style:background-color={hsvToHex(h, s, v, a)}
      ></div>
    </div>
