<script lang="ts">
  ...
  const month_names = Array.from({ length: 12 }, (_, month) => ({
    short: new Date(2024, month, 1).toLocaleDateString("en-US", { month: "short" }),
  ...
  const display_value = $derived(
    selected ? `${month_names[selected.month].long} ${selected.year}` : "Select month",
  );
...
<Popover.Root bind:open>
  <Popover.Trigger>
    {#snippet child({ props })}
      <Button {...props} variant="outline" class="w-full justify-between font-normal" {disabled}>
  ...
  <Popover.Content class="w-72 p-3" align="start">
    ...
    <div class="grid grid-cols-3 gap-1" role="grid" aria-label={`Months in ${display_year}`}>
