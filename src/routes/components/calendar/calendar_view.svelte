<script lang="ts">
  ...
  let visible_period_label = $derived(format_period_label(visible_date, view_mode));
...
<section class="flex min-w-[820px] flex-col gap-4 p-5" aria-label="Task due date calendar">
  <header
  ...
  >
    <div>
      <div class="flex items-center gap-2">
        ...
        <h2 class="text-xl font-semibold tracking-tight">{visible_period_label}</h2>
      </div>
      <p class="mt-1 text-sm text-muted-foreground">
        {due_task_count} {due_task_count === 1 ? "task" : "tasks"} with due dates
        <span class="mx-1.5" aria-hidden="true">·</span>
        <span class="font-medium text-foreground">
          {current_period_task_count}
          {current_period_task_count === 1 ? "entry" : "entries"} this {view_mode}
          {#if current_period_preview_count > 0}
            <span class="font-normal text-muted-foreground">
              ({current_period_preview_count} recurring
              {current_period_preview_count === 1 ? "preview" : "previews"})
            </span>
          {/if}
        </span>
      </p>
      <div class="mt-2 flex flex-wrap items-center gap-3 text-[11px] text-muted-foreground" aria-label="Calendar item legend">
        <span class="inline-flex items-center gap-1.5">
          <span class="size-2.5 rounded-sm border-2 border-foreground/60 bg-card shadow-xs"></span>
          Active
        </span>
        <span class="inline-flex items-center gap-1.5 opacity-70">
          <span class="size-2.5 rounded-sm border border-dashed border-muted-foreground/60 bg-transparent"></span>
          Recurring preview
        </span>
        <span class="inline-flex items-center gap-1.5 opacity-55">
          <span class="size-2.5 rounded-sm border border-dotted border-muted-foreground/50 bg-muted/30"></span>
          Archived
        </span>
      </div>
    </div>
    ...
    <div class="flex items-center gap-2">
      {#if !read_only}
        <Button variant="outline" size="sm" class="gap-1.5" onclick={() => { export_dialog_open = true; }}>
          <DownloadIcon class="size-3.5" />
          Export
        </Button>
      {/if}
      <Button variant="outline" size="sm" onclick={go_to_today}>Today</Button>
      <div class="flex overflow-hidden rounded-md border bg-background shadow-xs">
        <Button
          variant="ghost"
          size="icon-sm"
      ...
      <div class="flex overflow-hidden rounded-md border bg-background shadow-xs" aria-label="Calendar view">
        <Button
      ...
      {#if !read_only}<DropdownMenu.Root>
        ...
        <DropdownMenu.Content align="end" class="min-w-48">
          ...
          <DropdownMenu.CheckboxItem bind:checked={show_archived} closeOnSelect={false}>
            Show archived tasks
          </DropdownMenu.CheckboxItem>
  ...
  <DragDropProvider onDragStart={handle_drag_start} onDragEnd={handle_drag_end}>
    ...
    {#each weekdays as weekday}
      <div class="bg-muted/80 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
        {weekday}
      </div>
    {/each}
    ...
    {#each calendar_days as day (day.key)}
      ...
      >
        <div class="mb-1.5 flex items-center justify-between">
          ...
          <div class="flex items-center gap-1">
            {#if day.tasks.length > 0}
              <span class="text-[11px] tabular-nums text-muted-foreground">{day.tasks.length}</span>
            {/if}
            {#if !read_only}<Button
              variant="ghost"
              size="icon-sm"
        ...
        <div class="min-h-0 flex-1 overflow-y-auto pr-0.5 [scrollbar-width:thin]">
                ...
                >
                  <DropdownMenu.Root>
                    ...
                    >
                      ...
                      {#if entry.archived}
                        <DropdownMenu.Item onclick={() => ask_to_unarchive(entry.task)}>
                          <UndoIcon />
                          Unarchive Task
                        </DropdownMenu.Item>
                      {:else if !entry.preview}
