...
<div class="flex flex-wrap items-center gap-2">
    <Popover.Root bind:open={date_open}>
        ...
        <Popover.Content class="w-auto overflow-hidden p-0" align="start">
            <Calendar type="single" bind:value={date} onValueChange={set_date} captionLayout="dropdown" />
        </Popover.Content>
    </Popover.Root>
    ...
    {#if value}
        ...
        >
            ...
            <Popover.Content class="w-auto p-3" align="start">
                <div class="space-y-3">
                    <TimePicker bind:time view="dotted" minuteStep={5} selectHours={true} showSeconds={false} />
                    <div class="flex justify-end">
                        <Button size="sm" onclick={() => { time_open = false; }}>Done</Button>
                    </div>
                </div>
            </Popover.Content>
        </Popover.Root>
