...
export function build_share_snapshot(
  ...
  const shared_columns = columns
    ...
    .map((column) => ({
      ...
      color: column.color,
      tasks: column.tasks
        .filter((task) => selected_tasks.has(task.id))
        .map(serialize_task),
    }));
...
export function group_enabled_shares_by_board(
  shares: ManagedShare[],
...
export function save_managed_share(
  share: ManagedShare,
  signature: string,
): void {
...
export async function publish_share(
  ...
  const result = await invoke<{
    ...
    preferredPort:
      existing_share_port(existing) ?? requested?.preferred_port ?? null,
  });
  return {
    ...result,
