...
describe("board sharing", () => {
    ...
    it("publishes only selected active columns", () => {
        ...
        const private_column = create_column("column_3", "Private", "", [create_task("task_4", "Secret")]);
        const snapshot = build_share_snapshot([todo, private_column], [todo.id], [todo.tasks[0].id], " Roadmap ", new Date("2026-08-22T12:00:00Z"));
    ...
    it("requires a title and at least one selected column", () => {
        ...
        expect(() => build_share_snapshot([column], [], [], "Board")).toThrow("Select at least one");
        expect(() => build_share_snapshot([column], [column.id], [], "  ")).toThrow("Enter a name");
    });

    it("updates shared tasks but does not add newly created tasks", () => {
        const column = create_column("column_1", "Todo", "", [create_task("task_2", "Draft")]);
        const selected_columns = [column.id];
        const selected_tasks = [column.tasks[0].id];
        const before = share_content_signature([column], selected_columns, selected_tasks, "Board");

        column.tasks.push(create_task("task_3", "New task"));
        expect(share_content_signature([column], selected_columns, selected_tasks, "Board")).toBe(before);

        column.tasks[0].title = "Published";

        expect(share_content_signature([column], selected_columns, selected_tasks, "Board")).not.toBe(before);
    });
    ...
    it("ignores changes to tasks that are not shared", () => {
        ...
        const before = share_content_signature([column], [column.id], [column.tasks[0].id], "Board");

        column.tasks[1].title = "Still private";

        expect(share_content_signature([column], [column.id], [column.tasks[0].id], "Board")).toBe(before);
    });
    ...
    it("reuses the complete address of an existing share", async () => {
        ...
        expect(invoke).toHaveBeenCalledWith("publish_lan_share", expect.objectContaining({
            shareId: existing.id,
    ...
    it("parses a previous share URL or a raw share ID", () => {
        expect(parse_requested_share_link("http://192.168.1.20:12345/share/j3V_BXrcsGwtsXv6XAD1jA")).toEqual({
            id: "j3V_BXrcsGwtsXv6XAD1jA",
    ...
    it("rejects malformed or unsafe previous share links", () => {
        expect(() => parse_requested_share_link("short")).toThrow("valid Cardbe share link");
        expect(() => parse_requested_share_link("https://192.168.1.20:12345/share/j3V_BXrcsGwtsXv6XAD1jA")).toThrow();
        expect(() => parse_requested_share_link("http://192.168.1.20/share/j3V_BXrcsGwtsXv6XAD1jA")).toThrow();
        expect(() => parse_requested_share_link("http://192.168.1.20:12345/share/j3V_BXrcsGwtsXv6XAD1jA?x=1")).toThrow();
    });
