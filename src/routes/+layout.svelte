<script lang="ts">
	...
	onMount(() => {
		const dismiss_startup = () => document.getElementById('app-startup')?.classList.add('is-ready');
		const waits_for_workspace = window.location.pathname === '/';
		if (waits_for_workspace) {
			window.addEventListener('cardbe:workspace-ready', dismiss_startup, { once: true });
		} else {
			dismiss_startup();
		}

		if (dev) {
			return () => window.removeEventListener('cardbe:workspace-ready', dismiss_startup);
		}
