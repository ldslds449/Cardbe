...
describe("LatestShareSyncQueue", () => {
    it("waits for a request queued behind an in-flight publish", async () => {
        ...
        await initial;
        await queued;
        expect(run).toHaveBeenLastCalledWith("share", "latest", expect.any(Function));
    ...
    it("keeps an error terminal when the latest request fails", async () => {
        ...
        const queue = new LatestShareSyncQueue<string>(async () => { throw error; }, vi.fn(), on_error);
    ...
    it("marks a late run stale after retirement and recreation", async () => {
        ...
        const queue = new LatestShareSyncQueue<string>(async (_id, value, is_current) => {
            if (value === "first") {
                await first.promise;
    ...
    it("ignores an old publish error after retirement and recreation", async () => {
        ...
        const on_error = vi.fn();
        const queue = new LatestShareSyncQueue<string>(async (_id, value) => {
            if (value === "first") {
                await first.promise;
                throw old_error;
            }
