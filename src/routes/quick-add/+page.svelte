<script lang="ts">
  ...
  interface BoardsState { active_board_id: number; boards: Array<{ id: number; name: string }>; }
  ...
  async function resize_window_to_content() {
    ...
    try {
      ...
      made_resizable = true;
      if (!mounted || !content_root?.isConnected) return;
      const content_height = Math.ceil(content_root.getBoundingClientRect().height);
  ...
  async function load_columns(reset_to_active = false) {
    ...
    try {
      ...
      const next_board_id = reset_to_active || !selected_board_id || !state.boards.some((board) => String(board.id) === selected_board_id)
        ? String(state.active_board_id)
        : selected_board_id;
      const next_columns = (await invoke<Array<ColumnSummary & { tasks: unknown[] }>>(
        "get_board_columns", { boardId: Number(next_board_id) },
      )).map(({ id, name }) => ({ id, name }));
      if (generation !== column_load_generation) return;
      boards = state.boards;
      ...
      const remembered = window.localStorage.getItem(`cardbe.quick-add.column-id.${next_board_id}`);
  ...
  async function save() {
    ...
    try {
      if (mode === "task") {
        ...
        window.localStorage.setItem(`cardbe.quick-add.column-id.${selected_board_id}`, column_id);
      } else {
      ...
      console.error(caught);
      error = mode === "task" && String(caught).includes("board")
        ? String(caught)
        : mode === "task" ? "Couldn't add the task. Please try again." : "Couldn't add the note. Please try again.";
      await tick();
      schedule_window_resize();
      title_input?.focus();
    } finally {
...
>
  ...
  <form class="flex flex-none flex-col gap-3" onsubmit={(event) => event.preventDefault()}>
    <div class={mode === "task" ? "grid gap-1.5" : undefined}>
      {#if mode === "task"}
        <label for="quick-add-task-title" class="text-xs font-medium text-muted-foreground">Task</label>
      {/if}
      <Input
    ...
    {#if mode === "task"}
      <div class="grid gap-1.5">
        <label for="quick-add-board" class="text-xs font-medium text-muted-foreground">Add to board</label>
        <select
          id="quick-add-board"
      ...
      <div class="grid gap-1.5">
        <label for="quick-add-column" class="text-xs font-medium text-muted-foreground">Add to column</label>
        <select
          id="quick-add-column"
    ...
    <div class="flex min-h-10 shrink-0 items-end justify-between gap-3 bg-background pt-1">
