...
describe("update check interval", () => {
    it("waits 24 hours after a successful check", () => {
        ...
        expect(shouldCheckForUpdate(storage, now + UPDATE_CHECK_INTERVAL_MS - 1)).toBe(false);
        expect(shouldCheckForUpdate(storage, now + UPDATE_CHECK_INTERVAL_MS)).toBe(true);
    });
});
