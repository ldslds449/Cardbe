...
describe("task transfer", () => {
    ...
    it("rejects a board export", async () => {
        await expect(parse_portable_task('{"columns":[]}')).rejects.toThrow(/supported Cardbe task/);
    });

    it("accepts surrounding whitespace around shared text", async () => {
        const shared = await serialize_portable_task(create_task("task_1", "Shared"));
        expect((await parse_portable_task(`  ${shared}\n`)).title).toBe("Shared");
    });

    it("rejects legacy JSON sharing text", async () => {
        const legacy = 'cardbe-task:{"format":"cardbe-task","version":1,"task":{"id":-1,"title":"Legacy","description":"","color":"","labels":[]}}';
        await expect(parse_portable_task(legacy)).rejects.toThrow(/supported Cardbe task/);
    });
