...
class BinaryWriter {
    ...
    byte(value: number) { this.chunks.push(Uint8Array.of(value)); }
...
class BinaryReader {
    ...
    string(): string { return decoder.decode(this.take(this.uint32())); }
...
function encode_task(task: Task): Uint8Array {
    ...
    const recurrence_code = task.recurrence?.frequency === "daily"
        ? 1
...
function decode_task(bytes: Uint8Array): Task {
    ...
    if (reader.byte() !== BINARY_VERSION) throw new Error("Unsupported task sharing version");
    ...
    if (label_count > MAX_COLLECTION_ITEMS) throw new Error("Invalid task label count");
    const labels = Array.from({ length: label_count }, () => reader.string());
    const item_count = reader.uint32();
    if (item_count > MAX_COLLECTION_ITEMS) throw new Error("Invalid task checklist count");
    ...
    if (recurrence_code !== 0) {
        const frequency = recurrence_code === 1
            ? "daily"
            : recurrence_code === 2
              ? "weekly"
              : recurrence_code === 3
    ...
    if (!reader.done()) throw new Error("The task sharing text contains unexpected data");
...
function from_base64url(value: string): Uint8Array {
    if (!/^[A-Za-z0-9_-]+$/.test(value)) throw new Error("Invalid task sharing text");
...
export async function parse_portable_task(shared_text: string): Promise<Task> {
    ...
    if (input.length > MAX_SHARED_TEXT_LENGTH) throw new Error("Task sharing text is too large");
    if (!input.startsWith(BINARY_PREFIX)) {
        throw new Error("This is not supported Cardbe task sharing text");
    }
    try {
        return decode_task(await gunzip(from_base64url(input.slice(BINARY_PREFIX.length))));
    } catch (error) {
        if (error instanceof Error && error.message.startsWith("Invalid task")) throw error;
